...
**Type id**: `bigquery` · **Engines**: `pydantic`
...
`core/nld/flow/execution/backend/bigquery_with_pydantic.py`
...
|----------|----------------|:--------------:|:-----------:|:---------:|:-------------------:|
| `by_key` | `impl/by_key/backend/bigquery_with_pydantic.py` | ✅ | ❌ | ✅ | ❌ |
| `by_source_tst` | `impl/by_source_tst/backend/bigquery_with_pydantic.py` | ✅ | ❌ | ✅ | ❌ |
...
- **`get-state`** — `get_processing_state` / `get_post_processing_state`
  are not overridden; the base raises `NotImplementedError`.
- **`compute`** — resolves the next run's processing state in memory
  from `retrieve_current_state`, so it works without the read
  accessors.
